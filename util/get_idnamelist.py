...
def save_vpc_mapping(session, filename):
    ...
    try:
        ...
        # VPC名でソート
        sorted_vpc_id_name_mapping = dict(sorted(vpc_id_name_mapping.items(), key=lambda x: x[1].lower()))
...
def save_securitygroup_mapping(session, filename):
    ...
    try:
        ...
        # セキュリティグループ名でソート
        sorted_sg_id_name_mapping = dict(sorted(sg_id_name_mapping.items(), key=lambda x: x[1].lower()))
