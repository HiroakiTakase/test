...
SERVICE_POLICY_LIST = ['bucketpolicy', 'iammanagedpolicy', 'vpcepolicy', 'kmspolicy', 'securitygroup']
...
if __name__ == '__main__':
    ...
    if args[1] in SERVICE_POLICY_LIST:
        policy_paths = diff_functions.create_folder_paths(dev_folder, prd_folder, diff_folder, policy)
        for policy_path in policy_paths:
            diff_functions.compare_policies(*policy_path)
        print(CREATE_DIFF_MSG)
    elif args[1] in IAM_POLICY_LIST:
        policy_paths = diff_functions.create_rolegroup_paths(dev_folder, prd_folder, diff_folder, policy)
        for policy_path in policy_paths:
            diff_functions.compare_policies(*policy_path)
        print(CREATE_DIFF_MSG)
    else:
