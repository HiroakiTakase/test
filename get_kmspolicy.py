    ...
    try:
        ...
        for response in paginator.paginate():
            for key in response['Keys']:
                alias_name = None
                key_id = key['KeyId']
                key_type = client.describe_key(KeyId=key['KeyId'])['KeyMetadata']['KeyManager']
                ...
                if key_type == KEY_TYPE_CUSTOMER:
                    ...
                    filtered_aliases = filter(lambda x: 'TargetKeyId' in x and x['TargetKeyId'] == key_id, aliases_response['Aliases'])
                    if filtered_aliases:
                        latest_alias = max(filtered_aliases, key=lambda x: x.get('LastUpdatedDate', 0), default=None)
                    ...
                    if alias_name:
                        ...
                        filename = './{0}/{1}/kmspolicy/{2}.json'.format(env, origin_profile, alias_name)
                    else:
                        filename = './{0}/{1}/kmspolicy/{2}.json'.format(env, origin_profile, key_id)
